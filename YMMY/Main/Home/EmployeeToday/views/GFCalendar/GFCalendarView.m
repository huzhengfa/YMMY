...
#define kCalendarBasicColor [UIColor colorWithRed:255.0 / 255.0 green:255.0 / 255.0 blue:255.0 / 255.0 alpha:1.0]
...
- (UIButton *)setupCalendarHeaderButtonWithFrame:(CGRect)frame {
    ...
    [backButton setTitle:@"< 上月" forState:UIControlStateNormal];
    [button addSubview:backButton];
    backButton.frame = CGRectMake(0, 0, 60, frame.size.height);
    ...
    [button addSubview:nextButton];
    nextButton.frame = CGRectMake(frame.size.width - 60, 0, 60, frame.size.height);
    ...
    [backButton addTarget:self action:@selector(backMonth) forControlEvents:UIControlEventTouchUpInside];
...
-(void)nextMonth{
    [self.calendarScrollView setContentOffset:CGPointMake(self.frame.size.width * 2, 0) animated:YES];
}
