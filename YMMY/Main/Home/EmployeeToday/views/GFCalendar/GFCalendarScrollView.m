...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if ([super initWithFrame:frame]) {
        ...
        self.selectYear = [formatter stringFromDate:_currentMonthDate].integerValue;
        formatter.dateFormat = @"MM";
        self.selectMonth = [formatter stringFromDate:_currentMonthDate].integerValue;formatter.dateFormat = @"dd";
        self.selectDay = [formatter stringFromDate:_currentMonthDate].integerValue;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionViewL) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            // 标识今天
            if ((monthInfo.month == self.selectMonth) && (monthInfo.year == self.selectYear)) {
                if (indexPath.row == self.selectDay + firstWeekday - 1) {
